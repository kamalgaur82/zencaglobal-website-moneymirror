...
(function () {
  ...
  function makeSupabase(cfg) {
    ...
    return {
      ...
      /* HTTP "phones in" — a per-phone heartbeat, uncapped by the Realtime
         connection limit. Falls back silently if the SQL isn't applied yet. */
      ping(s, u) {
        if (!client || !s || !u) return Promise.resolve();
        return client.rpc("zenca_ping", { s, u }).then(() => {}).catch(() => {});
      },
      presentCount(s) {
        if (!client || !s) return Promise.resolve(0);
        const cutoff = new Date(Date.now() - 30000).toISOString();
        return client.from("zenca_present").select("uid", { count: "exact", head: true })
          .eq("sid", s).gt("seen_at", cutoff)
